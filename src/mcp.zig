...
pub const Server = struct {
    ...
    fn handleToolsList(self: *Server, request: JsonRpcRequest) !void {
        ...
        try tools.append(.{
            .name = "hover",
            .description = "Get hover information at a specific position in a Zig file",
            .inputSchema = try self.createHoverSchema(),
        });

        // Go to definition tool
        try tools.append(.{
            .name = "definition",
            .description = "Go to definition of symbol at a specific position",
            .inputSchema = try self.createDefinitionSchema(),
        });
        ...
        try tools.append(.{
            ...
            .inputSchema = try self.createCompletionsSchema(),
        });
    ...
    fn createHoverSchema(self: *Server) !json.Value {
        const schema = .{
            .type = "object",
            .properties = .{
                .uri = .{
    ...
    fn createDefinitionSchema(self: *Server) !json.Value {
        return self.createHoverSchema(); // Same schema
    }

    fn createCompletionsSchema(self: *Server) !json.Value {
        return self.createHoverSchema(); // Same schema
    }
