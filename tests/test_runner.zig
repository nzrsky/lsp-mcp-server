...
fn givenMcpServerIsAvailable(world: *bdd.World, matches: [][]const u8) !void {
    ...
    const server = try bdd.McpServerProcess.init(world.allocator, "./zig-out/bin/lsp-mcp-server", @constCast(&[_][]const u8{ "--server", "zls" }));
...
fn thenResponseShouldContainServerCapabilities(world: *bdd.World, matches: [][]const u8) !void {
    ...
    }, world.allocator, response, .{}) catch |err| {
